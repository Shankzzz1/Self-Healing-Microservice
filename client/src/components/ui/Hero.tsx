...
const Hero = () => {
  ...
  const productImages = [
    ...
    { url: "https://images.unsplash.com/photo-1544966503-7cc5ac882d5f?w=280&h=320&fit=crop", alt: "Sneakers" },
        ...
        <div className="grid lg:grid-cols-2 gap-12 w-full items-center">
          ...
          <div ref={textRef} className="space-y-8 max-w-xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Button 
                size="lg" 
                className="bg-white text-black hover:bg-gray-100 font-semibold px-8 py-6 text-lg group transition-all duration-300"
              >
                Shop Collection
              ...
              <Button 
                variant="outline" 
                size="lg" 
                className="border-gray-600 text-black hover:bg-white hover:text-black px-8 py-6 text-lg transition-all duration-300"
              >
                View Lookbook
              </Button>
            </div>
          ...
          <div ref={imagesRef} className="relative h-[600px] lg:h-[700px]">
            <div className="columns-3 gap-4 h-full">
              {productImages.map((image, index) => (
                ...
                >
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent rounded-xl opacity-0 hover:opacity-100 transition-opacity duration-300">
                    <div className="absolute bottom-4 left-4 right-4">
                      <p className="text-white font-semibold text-sm">{image.alt}</p>
                      <p className="text-gray-300 text-xs">Premium Collection</p>
                    </div>
                  </div>
                </div>
              ))}
            ...
            {/* Decorative elements */}
            <div className="absolute -top-4 -right-4 w-24 h-24 border border-gray-800 rounded-full opacity-30"></div>
            <div className="absolute -bottom-8 -left-8 w-32 h-32 border border-gray-800 rounded-full opacity-20"></div>
          </div>
        </div>
      </div>
      
      {/* Background decoration */}
      <div className="absolute top-0 right-0 w-1/3 h-full bg-gradient-to-l from-gray-900/20 to-transparent pointer-events-none"></div>
    </div>
  );
};
